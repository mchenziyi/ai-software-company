...
| **Secretary**（秘书 Agent） | 会议记录、Todo 列表生成与追踪、跨轮次状态管理 | ❌ 不参与内容决策、不投票 |

#### 秘书 Agent 设计

**定位**：秘书 Agent 不参与业务决策，不投票。它是会议的"记录员 + 追责者"。
...
| **追踪 Todo 状态** | 下一轮会议前，比对上一轮 Todo 和当前代码/产物状态，标记每个 Todo 为 done / pending / reverted |
| **发现遗漏** | 如果上一轮的 Todo 在下一轮没有被提及或解决，主动标记并提醒 Moderator |
...
```json
{
  ...
  "items": [
    {"id": 1, "desc": "DELETE 同时支持 body 和 query 传 version", "status": "done", "evidence": "handler.go:130"},
    {"id": 2, "desc": "/health 503 使用 ErrorResponse 格式", "status": "done", "evidence": "router.go:49"},
    {"id": 3, "desc": "PATCH version 从请求体读取而非 query", "status": "pending", "evidence": null}
  ]
}
```
...
生成 / 更新 Todo 列表（.aisc/tasks/）
    ↓
下一轮会议前 → 秘书 Agent 读取代码 → 验证 Todo 状态
    ↓
将"已完成 / 未完成 / 遗漏"报告注入 Moderator 的 prompt
```

**LLM 能力要求**：Standard 级别（不需要强推理，核心是结构化提取 + 状态匹配）
