...
| Secretary | Standard | 结构化提取、状态追踪、上下文压缩，不做内容裁决 |
...
| **Secretary**（秘书 Agent） | 会议记录、Todo 生成与生命周期追踪、跨轮次状态报告 | ❌ 不读代码、不判断质量、不参与投票和内容裁决 |

#### 秘书 Agent 设计

**定位**：秘书 Agent 不参与业务决策，不投票。不读代码、不判断代码质量，不做事实验证。它是会议的"流程记账员 + 上下文压缩器"。

**权限边界**：可以读 meeting / decision / artifact summary / Owner 修订说明，写 `.aisc/tasks/`。不可以读代码库，不可以直接判定 todo 是否真的修好。

**核心职责**：

| 职责 | 说明 |
|------|------|
| **记录会议** | 每轮评审会议结束后，读取所有 reviewer 意见 + Moderator 决策，生成结构化会议纪要 |
| **生成 Todo 列表** | 从 Decision 的 action_items 中提取可追踪的子任务，写入 `.aisc/tasks/{stage-id}-round{N}.json`（如 `stage-backend-round2.json`） |
| **追踪 Todo 生命周期** | 下一轮会议前，比对上一轮 Todo、Owner 修订说明和本轮评审结论，更新每个 Todo 的状态 |
| **发现遗漏** | 如果上一轮的 Todo 在本轮会议上没有被提及、被声称完成但 Reviewer 仍有意见、或持续多轮未关闭，主动标记并提醒 Moderator |
| **双视图输出** | 人类视图（markdown 摘要，写进 meeting.md）和 LLM 视图（结构化 JSON，注入下一轮的 Moderator prompt） |

**Todo 状态机**（五态流转）：

```
pending  →  claimed_done  →  confirmed_done
                ↓
           still_open（Owner 声称完成但 Reviewer 本轮认为仍未解决）
                
pending  →  dropped（Moderator 明确取消）
```
...
| 状态 | 含义 | 触发条件 |
|------|------|----------|
| `pending` | 待处理 | 新创建的 Todo |
| `claimed_done` | Owner 声称已完成 | 修订说明中标记为此项已处理 |
| `confirmed_done` | 确认关闭 | 本轮评审无人提及或明确标注已解决 |
| `still_open` | 声称完成但未接受 | 说修了但 Reviewer 认为未解决，或本轮继续提到 |
| `dropped` | 明确取消 | Moderator 在决策中标注作废 |

**Freeze 条件**：所有非 `dropped` 的 Todo 均为 `confirmed_done`，且本轮无新增 blocker。

**Todo 文件格式**（`.aisc/tasks/stage-backend-round2.json`）：

```json
{
  "stage": "stage-backend",
  "round": 2,
  "created_from": "meeting-002",
  "items": [
    {"id": 1, "desc": "DELETE 同时支持 body 和 query 传 version", "status": "confirmed_done", "evidence": "本轮 tech-lead + qa 均确认已解决"},
    {"id": 2, "desc": "/health 503 使用 ErrorResponse 格式", "status": "confirmed_done", "evidence": "本轮 Moderator 裁决已关闭"},
    {"id": 3, "desc": "PATCH version 从请求体读取而非 query", "status": "still_open", "evidence": "Owner 声称已修复，tech-lead 指出仍未生效"}
  ]
}
```
...
```
Meeting 结束 → Moderator 裁决
    ↓
秘书 Agent 读取 meeting.md + decision JSON
    ↓
生成 / 更新 Todo 列表（.aisc/tasks/）
    ↓
Owner 修订完成 → 秘书 Agent 读取修订说明
    ↓
下一轮会议 → 秘书 Agent 读取本轮评审结论
    ↓
更新 Todo 状态 → 将追踪报告注入下一轮 Moderator prompt
```

**Owner 修订说明的存放位置**：修订说明由 Owner Agent 在 revise 阶段写入 `docs/{artifact}.md` 的顶部 `changes` 字段。秘书 Agent 读取 `changes` 来标记 `claimed_done`。Reviewer 验证时对照 `changes` 和实际修改。

**LLM 能力要求**：Standard 级别（核心是结构化提取 + 生命周期追踪，不需要强推理）
...
| `write_task_state` | 写入 `.aisc/tasks/` 下的 Todo 状态文件（仅秘书 Agent 拥有） |

> 注：DevOps 和 Documentation 不参与大多数评审会议，只在各自 Stage 被调用。秘书 Agent 的 `write_task_state` 仅限写 `.aisc/tasks/`；不读代码、不做事实验证、不参与投票。
