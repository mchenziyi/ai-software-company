...
| **生成 Todo 列表** | 从 Decision 的 action_items 中提取可追踪的子任务，写入 `.aisc/tasks/{stage}-round{N}.json` |
...
pending  →  claimed_done  →  confirmed_done
                ↓
           still_open（本轮仍被指出未解决）
                
pending  →  dropped（Moderator 明确取消）
                
confirmed_done 多条 = 基本可以 freeze
```

**Todo 文件格式**（`.aisc/tasks/backend-round2.json`）：
